...
D3DSample::D3DSample( )
:g_pD3D( NULL ), g_pd3dDevice( NULL ), g_pMesh( NULL ), g_pMeshMaterials( NULL ), g_pMeshTextures( NULL ), g_dwNumMaterials(0L)
{
}
...
HRESULT D3DSample::InitD3D( HWND hWnd )
{
...
//-----------------------------------------------------------------------------
HRESULT D3DSample::InitGeometry( )
{
		...
		&g_pMesh ) ) )
	{
		...
		if ( FAILED( D3DXLoadMeshFromX( L"..\\Tiger.x", D3DXMESH_SYSTEMMEM,
			...
			&g_pMesh ) ) )
		{
			MessageBox( NULL, L"Could not find tiger.x", L"Meshes2.exe", MB_OK );
			return E_FAIL;
		}
	}
			 ...
			 lstrlenA( d3dxMaterials[i].pTextureFilename ) > 0 )
		{
				...
				&g_pMeshTextures[i] ) ) )
			{
				...
				if ( FAILED( D3DXCreateTextureFromFileA( g_pd3dDevice,
					...
					&g_pMeshTextures[i] ) ) )
				{
					MessageBox( NULL, L"Could not find texture map", L"Meshes2.exe", MB_OK );
				}
			}
		}
	}
...
//-----------------------------------------------------------------------------
LRESULT WINAPI D3DSample::MsgProc( HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam )
{
	switch ( msg )
	{
		case WM_DESTROY:
